...
void defaultProgressCallback( unsigned int bytesRead, unsigned int totalBytesRead )
{
}

static void* pProgressCallback = defaultProgressCallback;
...
int http_fetch(const char *url_tmp, char **fileBuf)
{
	...
	BOOL bFound = FALSE;
	http_progress_callback pCallback = 0;
	int i,
		ret = -1,
		tempSize,
		selectRet,
	...
	*/  do
	{
		...
		if(charIndex == NULL)
		{
			...
			*	request */ 
			...
			if(_checkBufSize(&requestBuf, &bufsize, tempSize) ||
				sprintf_s(requestBuf, bufsize, "GET / %s\r\n", HTTP_VERSION) < 0)
			{
				free(url);
				free(requestBuf);
				errorSource = ERRNO;
		...
		else
		{
			...
			if(_checkBufSize(&requestBuf, &bufsize, tempSize) ||
				sprintf_s(requestBuf, bufsize, "GET %s %s\r\n",
		...
		*	hurt anything */
		...
		while ( !bFound )
		{
			sock = makeSocket(host, iIndex);		/* errorSource set within makeSocket */
	...
	*/
	...
	while(ret > 0)
	{
		...
		{
			pCallback = (http_progress_callback)pProgressCallback;
			pCallback( ret /* Bytes Read */, bytesRead /* Total Bytes Read */ );
		}
...
* Added by VRocker
*/
void http_setProgressCallback( void* callback )
{
	if ( callback )
		pProgressCallback = callback;
	else 
		pProgressCallback = defaultProgressCallback;
}
...
*	-1 on error
*/
int makeSocket(const char *host, unsigned int i)
{
	...
	while ( hp->h_addr_list[ i ] != 0 )
	{
		...
		memcpy((char *)&sa.sin_addr, (char *)hp->h_addr_list[i++], hp->h_length);
